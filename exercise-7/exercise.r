...
lowest.ratio.team <- filter(team.data, Ratio == min(Ratio)) %>%
                     select(team.data, Team)
...
team.data <- mutate(team.data, ASTperGame = AST / G) %>%
             arrange(-ASTperGame)
...
better.shooters <- BetterShooters("Orlando Magic", "Minnesota Timberwolves")
